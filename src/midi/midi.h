...
#pragma pack(pop)
	...
	struct ChannelNoteCollector : public EventReceiver
	{
		...
		uint16_t velocitys[128];
		Time times[128];
		std::function<void(const NOTE&)> receiver;

		ChannelNoteCollector(Channel channel, std::function<void(const NOTE&)> receiver) :
			channel(channel), receiver(receiver)
		{
			for (uint16_t& var : velocitys)
			{
				var = 420;
	...
	public:
		...
		EventMulticaster(std::vector<std::shared_ptr<EventReceiver>> filters) :
			filters(filters) { };
